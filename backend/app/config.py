...
class Settings:
    ...
    enrollment_token_days: int = int(os.getenv("ENROLLMENT_TOKEN_DAYS", "1"))
    ...
    match_auto_threshold: float = float(os.getenv("MATCH_AUTO_THRESHOLD", "0.86"))
    # Retained for deployment compatibility; matching is automatic-only and
    # uses match_auto_threshold as its sole similarity cutoff.
    match_review_threshold: float = float(os.getenv("MATCH_REVIEW_THRESHOLD", "0.86"))
    match_runner_up_margin: float = float(os.getenv("MATCH_RUNNER_UP_MARGIN", "0.10"))
    ...
    threshold_profile_version: str = os.getenv("THRESHOLD_PROFILE_VERSION", "strict-auto-v2")
