...
export default function Enrollment() {
  ...
  async function camera() {
    try {
      ...
      setError(`Camera unavailable: ${requestError.message}. You can choose a selfie file instead.`);
    }
  }
  ...
  return <div className="public-shell"><main className="public-card card">
    ...
    <label className="consent-row"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} /><span>I consent to FDX processing this selfie into a face embedding solely to find my photographs for this event. The data expires with the event retention policy.</span></label>
    <button className="btn primary" disabled={!image || !consent || submitting} onClick={submit}>{submitting ? "Verifying face…" : "Submit secure enrollment"}</button>
  </main></div>;
}
