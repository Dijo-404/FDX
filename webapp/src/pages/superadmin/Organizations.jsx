  ...
  return (
    <div className="form-grid single">
      <div className="field">
        ...
        <input
          type="number"
          min={Math.max(1, organization.storageUsedGB)}
          value={quota}
          onChange={(event) => setQuota(event.target.value)}
          onBlur={() =>
            Number(quota) >= Math.max(1, organization.storageUsedGB) &&
            Number(quota) !== organization.storageLimitGB &&
            update(organization.id, { storageLimitGB: Number(quota) })
          }
        />
      </div>
      <div className="field">
        ...
        <input
          ...
          onBlur={() =>
            Number(retention) >= 1 &&
            Number(retention) <= 3650 &&
            Number(retention) !== organization.retentionDays &&
            update(organization.id, { retentionDays: Number(retention) })
          }
        />
      </div>
      <div className="field">
        ...
        <input
          ...
          onBlur={() =>
            ...
            update(organization.id, { expiry: expiry || null })
          }
        />
      </div>
    </div>
  ...
  return (
    <div className="page">
            ...
            <tbody>
              {visible.map((organization) => (
                <tr
                  key={organization.id}
                  className={
                    selected?.id === organization.id ? "row-selected" : ""
                  }
                  onClick={() => setSelectedId(organization.id)}
                >
                  <td>
                    <div className="table-identity">
                      <span className="org-avatar small">
                  ...
                  <td>
                    <div className="mini-usage">
                      <span>
                        <i
                          style={{
                            width: `${Math.min(100, (organization.storageUsedGB / organization.storageLimitGB) * 100)}%`,
                          }}
                        />
                      </span>
                  ...
                  <td>{organization.nextDataExpiry ?? "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
          {!visible.length ? (
            <p className="empty-note">No organizations found.</p>
          ) : null}
        </section>
        {selected ? (
          <aside className="card section organization-detail">
            ...
            <div className="gauge-row">
              ...
              <Gauge
                value={Math.max(
                  0,
                  selected.storageLimitGB - selected.storageUsedGB,
                )}
                max={selected.storageLimitGB}
                label="Remaining"
                sublabel={`${Math.max(0, selected.storageLimitGB - selected.storageUsedGB)} GB`}
              />
            </div>
            <PolicyEditor
              key={selected.id}
            ...
            <div className="detail-stat-row">
              ...
              <div>
                <p className="detail-stat-label">Retention</p>
                <p className="detail-stat-value">{selected.retentionDays}d</p>
              </div>
            </div>
      ...
      <Modal
        ...
        description="Provision a college or company tenant with its own policy boundaries."
        footer={
          <>
            <button className="btn" onClick={() => setOpen(false)}>
              Cancel
            </button>
            <button className="btn primary" form="organization-form">
              Create organization
            </button>
          </>
        }
